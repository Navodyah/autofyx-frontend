...
import { ArrowLeft, Save, Edit, Car } from 'lucide-react';

export default function EditVehicleClassPage() {
  ...
  useEffect(() => {
    if (!id || id === 'undefined') {
      alert('Invalid vehicle class ID');
      setLoading(false);
      return;
    }
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`http://127.0.0.1:8000/vehicle_classes/${id}`);
        setClassName(response.data.class_name);
        setLoading(false);
      } catch (error) {
        console.error(error);
        alert('Vehicle Class not found');
        setLoading(false);
      }
    };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.put(`http://127.0.0.1:8000/vehicle_classes/${id}`, {
        class_name: className
      });

      if (response.status === 200) {
        alert("Vehicle Class Updated Successfully!");
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600 font-semibold">Loading vehicle class data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-8">
      <div className="max-w-2xl mx-auto">
        {/* Back Button */}
        <Link href="/admin/catalog/vehicles_class">
        ...
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-orange-500 to-orange-600 px-8 py-6">
            <div className="flex items-center gap-3 text-white">
              ...
              <div>
                <h1 className="text-3xl font-bold">Edit Vehicle Class</h1>
                <p className="text-orange-100 mt-1">Update vehicle class information (ID: {id})</p>
              </div>
            </div>
          </div>
        ...
        <div className="mt-6 bg-orange-50 border-2 border-orange-200 rounded-xl p-6">
          ...
          <ul className="text-sm text-orange-800 space-y-1">
            ...
            <li>• Ensure the class name is descriptive and accurate</li>
