...
import { Plus, Edit, Trash2, Search, Fuel, Zap, BarChart3 } from 'lucide-react';
...
export default function FuelTypesListPage() {
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Are you sure you want to delete this fuel type?')) {
      try {
        ...
        await axios.delete(`http://127.0.0.1:8000/fuel_types/${id}`);
        setData(data.filter((item) => item.fuel_type_id !== id));
  ...
  const filteredData = data.filter((item) =>
    item.fuel_type_name.toLowerCase().includes(searchTerm.toLowerCase())
  );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-8">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-500 mt-2">Manage vehicle fuel type specifications</p>
            </div>
            <Link href="/admin_dashboard/catalog/FuelType/new">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-8">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search fuel types by name..."
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl shadow-xl p-6 text-white">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-blue-100 text-sm">Total Fuel Types</p>
                <p className="text-3xl font-bold mt-1">{data.length}</p>
              </div>
              <Fuel className="w-12 h-12 opacity-80" />
            </div>
          </div>
                  ...
                  {filteredData.length === 0 ? (
                    <tr>
                      <td colSpan={3} className="px-6 py-12 text-center text-gray-500">
                        ...
                        <p className="text-lg font-semibold">No fuel types found</p>
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-center">
                          <div className="flex items-center justify-center gap-2">
                            <Link href={`/admin_dashboard/catalog/FuelType/${item.fuel_type_id}`}>
                            ...
                            <button 
                              onClick={() => handleDelete(item.fuel_type_id)}
